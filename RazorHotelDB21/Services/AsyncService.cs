...
namespace RazorHotelDB21.Services {
    public abstract class AsyncService<T> : IAsyncService<T> {
        ...
        /// </summary>
        protected static string connectionString = "Data Source=(localdb)\\MSSQLLocalDB;Initial Catalog=HotelDbtest2;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False";
        /// <summary>
        ...
        public AsyncService(string relationalName, string[] relationalKeys, string[] relationalAttributes) {
            _relationalName = relationalName;
            _relationalKeys = relationalKeys;
            _relationalAttributes = relationalAttributes;
        }

        string IAsyncService<T>.RelationalName {
            get { return _relationalName; }
            set { _relationalName = value; }
        }
        string[] IAsyncService<T>.RelationalKeys {
            get { return _relationalKeys; }
            set { _relationalKeys = value; }
        }
        string[] IAsyncService<T>.RelationalAttributes {
